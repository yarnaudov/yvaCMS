...
class Banner extends CI_Model {
    ...
    function _image($banner)
    {
        ...
        $banner_html  = '<a href="'.site_url($this->gonfig->item('default_controller').'/banners/'.$banner['id']).'?url='.$banner['params']['link'].'" target="blank" >';
	...
	foreach($html->find('a') as $key => $a){
	    ...
	    $a->href = site_url($this->gonfig->item('default_controller').'/banners/'.$banner['id']).'?url='.$a->href;
